...
CREATE TABLE IF NOT EXISTS `bdJoyeria`.`producto` (
  ...
  CONSTRAINT `FKIDVenta`
    FOREIGN KEY (`IDVenta`)
    REFERENCES `bdJoyeria`.`venta` (`IDventa`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- Ejemplos de ventas
-- -----------------------------------------------------
INSERT INTO venta (fecha, cantidadArticulos, PrecioVenta, DireccionFacturacion, DNIUsuario) VALUES ('2021-10-15','1','100','Calle perico 5','12345678C'); 

-- -----------------------------------------------------
-- Añadimos a un producto el ID de la venta
-- -----------------------------------------------------
ALTER TABLE `producto` DROP FOREIGN KEY `FKIDVenta`;  
UPDATE producto SET IDVenta=(SELECT IDventa FROM venta WHERE DireccionFacturacion='Calle perico 5') WHERE TipoPieza='Anillo';
ALTER TABLE producto ADD CONSTRAINT FKIDVenta FOREIGN KEY (IDVenta) REFERENCES venta (IDventa) ON DELETE NO ACTION ON UPDATE NO ACTION;
